...
class IR_GET(IR_RX):
    ...
    def decode(self, _):
        ...
        if self.display:
            ...
            if not ok and near(burst[0], 2400) and near(burst[1],
                                                        600):  # Maybe Sony
                try:
            ...
            if not ok and near(burst[0], 2666) and near(burst[1],
                                                        889):  # RC-6?
                if near(duration, 22205) and near(burst[1], 889) and near(
                        burst[2], 444):
            ...
            if not ok and near(burst[0], 2000) and near(burst[1], 1000):
                if near(duration, 19000):
                    ...
                    print('Protocol start {} {} Burst length {} duration {}'.
                          format(burst[0], burst[1], lb, duration))
                    ok = True

            if not ok and near(burst[0], 4500) and near(
                    burst[1], 4500) and lb == 67:  # Samsung
                print('Samsung')
                ok = True

            if not ok and near(burst[0], 3500) and near(burst[1],
                                                        1680):  # Panasonic?
            ...
            if not ok:
                print(
                    'Unknown protocol start {} {} Burst length {} duration {}'.
                    format(burst[0], burst[1], lb, duration))
    ...
    def decode(self, _):
        try:
            ...
            if width > 3000:  # 4.5ms space for normal data
                ...
                raise RuntimeError(self.REPEAT if self.edge == 4 else
                                   self.BADREP)  # Treat REPEAT as error.
            else:
                raise RuntimeError(self.BADSTART)
            ...
            if addr != (
                (val >> 8) ^ 0xff) & 0xff:  # 8 bit addr doesn't match check
